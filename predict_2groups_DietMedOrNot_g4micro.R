...
load("/home/carlos/Documentos/CORALS/results_rstudio/results_Agosto25_1//DAA_Diet_MedOrNot/DeSEQ2/DESEQ2_all_MedVsProcOrWes_phyloseqList.RData")
load("/home/carlos/Documentos/CORALS/results_rstudio/results_Agosto25_1//DAA_Diet_MedOrNot/DeSEQ2/DESEQ2_all_MedVsProcOrWes_phobj_filt.RData")
load("/home/carlos/Documentos/CORALS/results_rstudio/results_Agosto25_1//DAA_Diet_MedOrNot/DeSEQ2/DESEQ2_all_MedVsProcOrWes.RData")


var2predict <- "Pattern_MedOrNot"
levs2pred <- c("Mediterranean", "Proc. or Western")
vars2pca <- c("Pattern_MedOrNot", "status_c2", "hospital", "Sex", "age_months_T0")
phseq_to_use <- "remove_tanda2" #names(daa_all) #[c(1,3,4,6,9,10)]
patnames <- c("Mediterranean", "Preprocessed", "Western")

opt <- restaurar(opt)
opt$out <- paste0(opt$out, "PredictDAA_MeditOrNot")
...
## for(i in phseq_to_use){
  ...
  #phobj_filt already loaded
  outdir <- paste0(opt$out, "PredictDAA_MeditOrNot/", i, "/")
  opt$reserva <- opt$out
  opt$out <- outdir
  if(!dir.exists(opt$out)) dir.create(opt$out)
  ...
  #all_model_results[[i]][["padj_taxa_res001"]] <- callDoAllModelsFromALLPCAs(all_pcas_adj001, name=paste0(i, "ConditionPadj001"), metadata=this_metadata, vars2pca=var2predict)
   ...
   PCs <- all_model_results[[i]][["padj_taxa_res"]]$varnames[1:2]
...
save(all_model_results, file=paste0(opt$out, "PredictDAA_MeditOrNot/all_model_results_withInd.RData"))
...
write_tsv(merged_df, file = paste0(opt$out, "250926_all_models_summary_MeditOrNot.tsv"))
...
ggsave(filename = paste0(opt$out, "DOTPLOT_", METRIC, "_MeditOrNot.pdf"), g0, width = 8, height = 3) #width = 12, height = 9
write_tsv(merged_df_mod, file = paste0(opt$out, "DOTPLOT_", METRIC, "_MeditOrNot.tsv"))
