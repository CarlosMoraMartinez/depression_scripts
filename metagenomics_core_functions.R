...
getAlphaDiversity <- function(phseq_obj, vars, qvars= c(),
  ...
  for(v in vars){
    ...
    if(correct_pvalues & num_comparisons*length(indices)>1){
      signif_levels_bonferroni <- c(signif_levels[1:3]/(num_comparisons), signif_levels[4]) #*length(indices)
    }else{
...
getAlphaDiversityCustomPlot <- function(phseq_obj, vars, qvars= c(),
                              opt = list(),
                              indices=c("Observed", "Chao1", "Shannon", "InvSimpson", "Fisher"),
                              name="AlphaDiversity",
                              signif_levels=c("***"=0.001, "**"=0.01, "*"=0.05, "ns"=1.1),
  ...
  divlong <- divtab %>%
    select(sampleID, all_of(vars), all_of(qvars), all_of(indices)) %>%
    gather("Index", "Value", all_of(indices)) %>%
  ...
  for(v in vars){
    #Si no lo convertimos a caracter, geom_signif/stat_signif fallan
    standa1 <-divlong[!is.na(divlong[, v]), ]

    divtab2 <- divtab[!is.na(divtab[, v]), ]
    divtab2[, v] <- as.character(divtab2[, v])

    comp <- combn(unique(as.character(standa1[, v])), 2, simplify = F)
    num_comparisons <- alphadif %>%
      filter(comparison != "all" &
               groups == v &
               variable == "Observed") %>%
    ...
    plots[[v]] <- ggplot(standa1, aes(x=!!sym(v), y=Value, col = !!sym(v)))+
      facet_wrap(~ Index, scales="free", nrow=1) +
      geom_violin() +
      geom_boxplot(aes_string(fill = v), alpha = 0.7, width=0.2,
                   fill="gray", outliers = FALSE) +
      #scale_color_manual(values = c("#ffafcc", "#90DBF4")) +
      #scale_fill_manual(values = c("#ffafcc", "#90DBF4")) +
      #scale_color_lancet() +
      scale_color_npg() +
      labs(title = v, x = '') +
      mytheme +
      ggsignif::stat_signif( test=test2show,
                            na.rm=T, comparisons = comp,
                            step_increase=0.06,
                            tip_length = 0.01,
                            map_signif_level=signif_levels_bonferroni,
                            vjust=0.3,
  ...
  for(v in qvars){
    ...
    #  dplyr::mutate(text = paste0("R^2=", as.character(round(r.squared, 2)), ", p=", as.character(round(p.value, 3)) ))
    standa1 <-divlong[!is.na(divlong[, v]), ]
    plots[[v]] <- ggplot(standa1, aes(x=!!sym(v), y=Value))+
      facet_wrap(~ Index, scales="free", nrow=1) +
...
######################## PERMANOVA
adonis2table <- function(mod1, var){
  aux <- data.frame(
    variable = var,
    DF_var=mod1$Df[1],
...
makePermanova <- function(phobj, dist_method = "bray", seed = 123,
  ...
  for(var in vars2test){
    ...
    if(length(unique(sampledf[!is.na(sampledf[, var]) , var])) > 1){
      ...
      res <-rbind(res, adonis2table(mod1, var))
    }
  }
...
makePermanovaFormulas <- function(phobj, formulas, dist_method = "bray", seed = 123,
                                  outname = "permanovas_mult.tsv", by=NULL, parallel=1){
  ## From https://deneflab.github.io/MicrobeMiseq/demos/mothur_2_phyloseq.html#permanova
  library(stringi)
  ...
  for(form in formulas){
    cat("Making Adonis2: ", as.character(form), "\n")
    set.seed(seed)
    mod1 <- adonis2(as.formula(form), data = sampledf, na.action=na.exclude,
                    by=by, parallel=parallel)
    res <-rbind(res, adonis2table(mod1, form))
    modelos[[form]] <- mod1
  }
  res <- res %>% dplyr::arrange(P)
  res$padj <- p.adjust(res$P, method="BH")
  readr::write_tsv(res, file=outname)
...
getDeseqContrastFromCategorical <- function(dds, lev_combin, opt, name, get_all_shrinkages=TRUE){
...
getDeseqContrastFromNumerical <- function(dds, nvarname, opt, name, get_all_shrinkages=TRUE){
...
getDeseqResults <- function(phobj, opt, name="", variables = c("Condition"), interact=FALSE,
                            doPoscounts = FALSE, all_combins=list(),
                            get_all_shrinkages=TRUE, get_all_contrasts=TRUE){
  if(interact == FALSE){
  ...
  if(interact == FALSE){
    ...
    if(!get_all_contrasts) all_combins <- all_combins[1]
    all_contrasts <- map(all_combins, \(lev_combin){
      if(lev_combin[2] == "NUMERIC"){
        getDeseqContrastFromNumerical(dds, lev_combin[1], opt, name, get_all_shrinkages=get_all_shrinkages)
      }else{
        getDeseqContrastFromCategorical(dds, lev_combin, opt, name, get_all_shrinkages=get_all_shrinkages)
      }
    })
...
make_all_heatmaps<- function(dearesults, df2plot, metadata, vars2heatmap, dds, opt){
  for(singleres in dearesults){
    ...
    cat("filtering pvalues lower than ", opt$pval, "; getting ", length(taxalist_praw), " and ", length(taxalist_padj), " taxa. \n")
...
deseq_full_pipeline <- function(phobj, name, vars2deseq, opt, interact=FALSE,  doPoscounts=FALSE,
                                all_combins = list(), plot_all = 1, deseqname = "DeSEQ2/", vars2heatmap=c(),
                                return_all=TRUE, get_all_shrinkages=TRUE, get_all_contrasts=TRUE){
  opt <- restaurar(opt)
  ...
  dearesults <- getDeseqResults(phobj, opt, name, variables = vars2deseq, interact = interact,
                                ...
                                get_all_shrinkages=get_all_shrinkages, get_all_contrasts=get_all_contrasts)
...
replaceOutliersByFunc <- function(vname, s_meta, p_lim=0.05, replace_fun="max"){
  aux <- detectOutliers(s_meta, dnames[8], 0.01)
  not_outliers <- s_meta %>%
    filter(!is.na(!!sym(vname))) %>%
    filter(! sampleID %in% aux$sampleID) %>%
    pull(!!sym(vname))
  if(replace_fun == "max"){
    fz <- max
  }else if(replace_fun == "mean"){
    fz <- mean
  }else if(replace_fun == "median"){
    fz <- median
  }else if(replace_fun == "min"){
    fz <- min
  }else{
    stop("Error: function to replace outliers not recognized\n")
  }
  s_meta <- s_meta %>%
    dplyr::mutate(!!vname := ifelse(sampleID %in% aux$sampleID, fz(not_outliers), .data[[vname]]))
  return(s_meta[[vname]])
}

replaceAllOutliers <- function(dnames, s_meta, p_lim=0.05, funct_name="max"){
  s_meta_no_outliers <- s_meta
  for(nn in dnames){
    s_meta_no_outliers <- s_meta_no_outliers %>%
      dplyr::mutate( !!nn :=  replaceOutliersByFunc(nn, ., p_lim, funct_name))
  }
  return(s_meta_no_outliers)
}
