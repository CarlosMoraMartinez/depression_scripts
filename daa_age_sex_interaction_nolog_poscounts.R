...
DANAME <- "DESeq2_AgeSexInteraction_noLogPoscounts/"
...
for(ff in names(designs)){
  ...
  if(!dir.exists(opt$out)) dir.create(opt$out)
  dds_all[[ff]] <- getDeseqResults(phobj_filt, opt = opt, name = ff, variables=NULL, formula=designs[[ff]], doposcounts=TRUE)
