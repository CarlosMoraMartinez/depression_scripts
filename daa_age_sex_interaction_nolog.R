...
DANAME <- "DESeq2_AgeSexInteraction_noLogNoPoscounts/"
...
designs <- list(
  Cond="~ Condition",
  BMI="~ BMI",
  CondSexIAge = "~ Condition + Sex * Age",
  BMISexIAge ="~ BMI + Sex * Age",
  CondBMISexIAge ="~ Condition + BMI + Sex * Age"
)
...
for(ff in names(designs)){
  ...
  opt <- restaurar(opt)
  opt$out <- paste0(opt$out,  DANAME, "/DESeq2_AgeSexInteraction_", ff, "/")
  opt$mincount <- 1
  opt$minsampleswithcount <- opt$minfreq * nrow(sample_data(all_phyloseq$remove_tanda2))

  if(!dir.exists(opt$out)) dir.create(opt$out)
  dds_all[[ff]] <- getDeseqResults(phobj_filt, opt = opt, name = ff, variables=NULL, formula=designs[[ff]], doposcounts=FALSE)
...
contrastlist2 <- list(
  ...
  dds_all$BMI$resdf,
  dds_all$BMISexIAge$all_contrasts$BMI$resdf,
  dds_all$CondBMISexIAge$all_contrasts$BMI$resdf
